  ...
  const rows = products.map((p, i) => `
    <div style="margin-bottom:28px;padding:20px;background:#fffdf9;border-radius:12px;border:1px solid #e8ddd0;">
      ...
      <div style="margin-top:8px;">
        ...
        ${p.bestStoreName ? `<div style="font-size:12px;color:#9a8878;margin-bottom:10px;">Best match at ${p.bestStoreName}</div>` : ''}
        <div style="text-align:right;">
          <a href="${p.buyLink}" target="_blank" style="display:inline-block;background:linear-gradient(135deg,#c8922a,#c4623a);color:white;font-weight:600;font-size:14px;padding:10px 24px;border-radius:50px;text-decoration:none;white-space:nowrap;">Shop This Gift →</a>
        </div>
      </div>
      ${(p.stores || []).length > 0 ? `
